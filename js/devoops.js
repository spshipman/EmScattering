...
//my functions
	...
	//define function for adding multiple small text forms relative to numberoflayers slider
	//addStructLayers is the div class
	...
	//define function for adding multiple small text forms relative to numberoflayers slider
	//addStructLayers is the div class
	// function updateLayers(counter) {
        
	// 	$("#addStructLayers").empty();      
	// 	$("#addStructLayers").append(counter);
...
function addNumLayers(divName, numInputs) {
  ...
  numOfLayers = parseInt(numInputs) + 2;
  var layersToAdd = numOfLayers;
...
function getIncomingMode() {
	...
	var mu = [];
	var length = [];
    var layers = parseInt(document.getElementById("ui-spinner").value)+2;
     ...
     console.log(length);
     crystal = new emScattering.PhotonicStructure1D(epsilon, mu, length);
     var incoming = crystal.incomingModes(omega, k1, k2);
     var mBack1x = incoming[0].eigenvalue.x;
     var mBack1y = incoming[0].eigenvalue.y;
     var mBack2x =  incoming[1].eigenvalue.x;
     var mBack2y = incoming[1].eigenvalue.y;
     var mFor1x = incoming[2].eigenvalue.x;
     var mFor1y = incoming[2].eigenvalue.y;
     var mFor2x = incoming[3].eigenvalue.x;
     var mFor2y = incoming[3].eigenvalue.y;
     ...
     $("#mode1").remove();
     $("#mode2").remove();
     newdiv.innerHTML = "<div id='mode1'><u>"+mFor1x+" + "+mFor1y+"i<br> "+mFor2x+" + "+mFor2y+"i</u></div>";
     newdiv2.innerHTML = "<div id='mode2'><u>"+mBack1x+" + "+mBack1y+"i<br> "+mBack2x+" + "+mBack2y+"i</u></div>";
...
function printFieldsChart(divName) {
    ...
    var layers = parseInt(document.getElementById("ui-spinner").value)+2;
     ...
     var crystal1 = new emScattering.PhotonicStructure1D(epsilon, mu, length);

     var fields = crystal1.determineField(o, k1, k2, [j1, j2, j3, j4]);
     interfaces = crystal1.materialInterfaces();
     
     

     var data = [];
     data.push(["z", document.getElementById("shownVal").value]);
     ...
     var options = {
        ...
        drawMyGoogleChart(data, options, divName, google.visualization.LineChart);
...
// 	chart.draw(data, chart_options);
// }
function drawMyGoogleChart(chart_data, chart_options, element, chart_type) {
	// Function for visualize Google Chart
	var data = google.visualization.arrayToDataTable(chart_data);
	var chart = new chart_type(document.getElementById(element));
